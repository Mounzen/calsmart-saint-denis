"""
CAL Smart / Logivia - Generateur de courrier .docx natif
Usage : python3 generate_docx.py <output_path>
       (le JSON d entree est lu sur stdin)

Le JSON doit contenir :
{
  "type": "courrier" | "fiche_candidat" | "fiche_cal",
  "titre": "Notification d attribution",
  "ref": "CO12345",
  "date_fr": "23 avril 2026",
  "destinataire": {
    "civilite": "Monsieur",
    "prenom": "Jean",
    "nom": "Dupont",
    "adresse_ligne1": "12 rue des Fleurs",
    "adresse_ligne2": "97400 Saint-Denis"
  },
  "objet": "Attribution d un logement social",
  "paragraphes": ["par 1", "par 2", ...],
  "signature_line": "Pour le Maire, l Adjoint delegue au Logement"
}

Genere un .docx valide (XML OOXML zippe) sans dependance externe.
"""
import sys
import json
import zipfile
...
def p_title(text, style="Titre"):
    return '<w:p><w:pPr><w:pStyle w:val="{}"/></w:pPr><w:r><w:t xml:space="preserve">{}</w:t></w:r></w:p>'.format(style, esc(text))
...
def p_meta_2col(left, right):
    """ Ligne avec une colonne a gauche et une a droite via tab. """
...
def p_separator():
    """ Paragraphe qui agit comme une ligne de separation. """
...
def build_docx(output_path, data):
...
def main():
    if len(sys.argv) < 2:
        sys.stderr.write('Usage: generate_docx.py <output_path> (JSON sur stdin)\n')
        sys.exit(2)
    output_path = sys.argv[1]
    data = json.loads(sys.stdin.read())
    build_docx(output_path, data)
    sys.stderr.write('OK ' + output_path + '\n')
